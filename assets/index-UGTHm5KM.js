(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`vinicius123`,t=`rocal_producoes_ts_v2`,n=[`m1`,`m2`,`m3`,`m4`,`m5`,`div`,`almox`],r={m1:`Máquina M1`,m2:`Máquina M2`,m3:`Máquina M3`,m4:`Máquina M4`,m5:`Máquina M5`,div:`Diversos`,almox:`Almoxarifado`},i=null,a=null,o=null,s=null,c=document.querySelector(`#app`);if(!c)throw Error(`Elemento #app não encontrado.`);var l=c;function u(){let e=new Date,t=e.getTimezoneOffset();return new Date(e.getTime()-t*6e4).toISOString().slice(0,10)}function d(e=`id`){return`${e}_${Date.now()}_${Math.random().toString(36).slice(2,8)}`}function f(e){let t=document.querySelector(e);if(!t)throw Error(`Elemento não encontrado: ${e}`);return t}function p(){try{let e=localStorage.getItem(t)||`[]`;return JSON.parse(e)}catch{return[]}}function m(e){localStorage.setItem(t,JSON.stringify(e))}function h(e){let t=Number(String(e).replace(`,`,`.`));return Number.isFinite(t)?t:0}function g(e){return{date:new Intl.DateTimeFormat(`pt-BR`,{weekday:`long`,day:`2-digit`,month:`2-digit`,year:`numeric`}).format(e),time:new Intl.DateTimeFormat(`pt-BR`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`}).format(e)}}function _(){l.innerHTML=`
    <div class="circuit">
  ...
  `,n.appendChild(a)}function S(){let e=f(`#turn-date`).value,t=f(`#turn-notes`).value.trim(),r={},i=t.length>0;return n.forEach(e=>{let t=Array.from(f(`#${e}-rows`).querySelectorAll(`.rowItem`)).map(e=>{let t=e.querySelector(`.input-code`)?.value.trim().toUpperCase()||``,n=h(e.querySelector(`.input-qty`)?.value||`0`);return(t||n>0)&&(i=!0),{cod:t||`S/ COD`,qtd:n}});r[e]={total:t.reduce((e,t)=>e+t.qtd,0),detalhes:t}}),{record:{data:e,observacoes:t,valores:r},hasAnyData:i}}function C(){let{record:e,hasAnyData:t}=S();if(!e.data)return z(`Defina uma data válida para o turno.`);if(!t)return z(`Preencha pelo menos uma produção, material ou observação.`);let n=p();if(i){let t=n.findIndex(e=>e.id===i);if(t===-1)return z(`Registro de edição não localizado.`);n[t]={...n[t],...e,atualizadoEm:new Date().toISOString()},m(n),j(),z(`Registro atualizado com sucesso.`)}else n.push({...e,id:d(`turno`),criadoEm:new Date().toISOString()}),m(n),z(`Produção gravada com sucesso.`);w(),T(),k(`historico`)}function w(){f(`#turn-date`).value=u(),f(`#turn-notes`).value=``,n.forEach(e=>b(e,2))}function T(){let e=f(`#view-historico`),t=p();e.innerHTML=`
    <div class="panelHead">
  ...
  `}function D(e,t){let n=e.valores?.[t]||{total:0,detalhes:[]},i=n.detalhes.filter(e=>e.cod!==`S/ COD`||e.qtd>0).map((e,t)=>`<div class="detailLine"><span>${t+1}. <b style="color:#fde047">${O(e.cod)}</b></span><b>${e.qtd}</b></div>`).join(``)||`<p style="color:#71717a;font-size:12px;font-style:italic">Sem movimentações</p>`;return`<div class="detailCard"><div class="detailHead"><b>${r[t]}</b><span>${n.total}</span></div>${i}</div>`}function O(e){return e.replace(/[&<>'"]/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,"'":`&#039;`,'"':`&quot;`})[e]||e)}function k(e){f(`#view-lancamento`).classList.toggle(`hidden`,e!==`lancamento`),f(`#view-historico`).classList.toggle(`hidden`,e!==`historico`),f(`#tab-lancamento`).classList.toggle(`active`,e===`lancamento`),f(`#tab-historico`).classList.toggle(`active`,e===`historico`),e===`historico`&&T()}function A(e){let t=p().find(t=>t.id===e);if(!t)return z(`Registro não localizado.`);i=e,f(`#turn-date`).value=t.data,f(`#turn-notes`).value=t.observacoes||``,f(`#form-title`).innerText=`Editar Registro de Turno`,f(`#edit-badge`).classList.remove(`hidden`),f(`#save-btn`).innerText=`Atualizar Lançamento no Banco`,f(`#cancel-edit-btn`).classList.remove(`hidden`),n.forEach(e=>{f(`#${e}-rows`).innerHTML=``;let n=t.valores?.[e]?.detalhes||[],r=Math.max(2,n.length);for(let t=0;t<r;t++)x(e,n[t])}),k(`lancamento`),window.scrollTo({top:0,behavior:`smooth`}),z(`Registro carregado para edição.`)}function j(){i=null,f(`#form-title`).innerText=`Registrar Lançamento`,f(`#edit-badge`).classList.add(`hidden`),f(`#save-btn`).innerText=`Gravar Produção no Banco`,f(`#cancel-edit-btn`).classList.add(`hidden`)}function M(e){let t=p(),n=t.findIndex(t=>t.id===e);if(n===-1)return z(`Registro não localizado.`);a={record:t[n],index:n},t.splice(n,1),m(t),T(),z(`Lançamento apagado do histórico.`,!0)}function N(){if(!a)return;let e=p();e.splice(a.index,0,a.record),m(e),a=null,T(),z(`Ação desfeita. Lançamento recuperado.`)}function P(){let e=p();if(!e.length)return z(`Não há lançamentos para exportar.`);let t=`Data,Criado em,Atualizado em,Maquina/Setor,Codigo,Quantidade,Total do Setor,Observacoes
