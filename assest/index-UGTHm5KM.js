const firebaseConfig = {
  ...
  appId: "1:747786696119:web:0db2c3065331b29bd32432",
  measurementId: "G-RG8TC64D64"
};
...
try {
  if (window.firebase && window.firebase.initializeApp) {
    ...
    bancoRef = window.firebase.firestore().collection("rocal").doc("producao_noturno");
  } else {
    console.error("Firebase SDK não carregou. Confira conexão e scripts do index.html.");
  }
} catch (erroFirebaseInit) {
  console.error("Erro ao iniciar Firebase:", erroFirebaseInit);
}

(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`vinicius123`,t=`rocal_producoes_ts_v2`,n=[`m1`,`m2`,`m3`,`m4`,`m5`,`div`,`almox`],r={m1:`Máquina M1`,m2:`Máquina M2`,m3:`Máquina M3`,m4:`Máquina M4`,m5:`Máquina M5`,div:`Diversos`,almox:`Almoxarifado`},i=null,a=null,o=null,s=null,c=document.querySelector(`#app`);if(!c)throw Error(`Elemento #app não encontrado.`);var l=c;function u(){let e=new Date,t=e.getTimezoneOffset();return new Date(e.getTime()-t*6e4).toISOString().slice(0,10)}function d(e=`id`){return`${e}_${Date.now()}_${Math.random().toString(36).slice(2,8)}`}function f(e){let t=document.querySelector(e);if(!t)throw Error(`Elemento não encontrado: ${e}`);return t}async function p(){try{if(!bancoRef)throw Error(`Firebase não inicializado.`);let e=await bancoRef.get();return e.exists&&Array.isArray(e.data().registros)?e.data().registros:[]}catch(e){console.error(`Erro ao carregar Firebase:`,e);typeof z===`function`&&z(`Erro ao carregar dados do Firebase. Confira internet, regras do Firestore e domínio autorizado.`);return[]}}async function m(e){try{if(!bancoRef)throw Error(`Firebase não inicializado.`);await bancoRef.set({registros:e,atualizadoEm:new Date().toISOString()},{merge:true})}catch(t){console.error(`Erro ao salvar Firebase:`,t);typeof z===`function`&&z(`Erro ao salvar no Firebase. Confira internet, regras do Firestore e domínio autorizado.`);throw t}}function h(e){let t=Number(String(e).replace(`,`,`.`));return Number.isFinite(t)?t:0}function g(e){return{date:new Intl.DateTimeFormat(`pt-BR`,{weekday:`long`,day:`2-digit`,month:`2-digit`,year:`numeric`}).format(e),time:new Intl.DateTimeFormat(`pt-BR`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`}).format(e)}}function _(){l.innerHTML=`
    <div class="circuit">
